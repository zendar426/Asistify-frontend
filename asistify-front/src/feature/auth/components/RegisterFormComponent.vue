...
import { ref } from 'vue'
import { type RegisterUserDto, useAuthStore } from '@/stores/authStore.ts'
...
function validateRut(fullRut: string) {
    ...
    if (!/^[0-9]+-[0-9kK]{1}$/.test(rutNoDot)) return false
...
function register() {
    if (checkData()) {
        ...
        const response = auth.registerUser(registerUser)
...
<template>
    <form class="max-w-xl mx-auto" @submit.prevent="register">
        <label class="text-red-500" v-show="showRegisterIncorrect"
        ...
        <div class="mb-5">
            ...
            <label class="text-red-500" v-show="showRutInvalid"
                >El rut ingresado no es válido.</label
            >
        </div>
        correo
        <div class="mb-5">
        ...
        <label class="text-red-500" v-show="showPasswordMismatch"
        ...
        <BaseButton variant="outline" size="lg" type="submit"> Registrarse </BaseButton>
    </form>
</template>
