...
export const useDocuments = () => {
  ...
  const filteredDocuments = computed(() => {
    ...
    if (searchQuery.value) {
      result = result.filter((doc) =>
        doc.original_name.toLowerCase().includes(searchQuery.value.toLowerCase()),
      )
    }

    if (documentTypeFilter.value) {
      result = result.filter((doc) => doc.document_type_id === documentTypeFilter.value)
    }
  ...
  const loadDocuments = async (enterpriseId: string) => {
    try {
      ...
      const docs = await documentService.getAllDocuments({ enterpriseId })
      documentStore.setDocuments(docs)

      const chunksCount = docs.reduce((acc, doc) => acc + doc.chunksCount, 0)
      documentStore.setTotalChunks(chunksCount)

      // simulete with the previous mounth
  ...
  const uploadDocument = async (enterpriseId: string, file: File) => {
    ...
    try {
      ...
      const newDocument = await documentService.createDocument(enterpriseId, file)
      documentStore.addDocument(newDocument)

      // recalculate total chunks after adding document
      const chunksCount = documentStore.documents.reduce((acc, doc) => acc + doc.chunksCount, 0)
      documentStore.setTotalChunks(chunksCount)
  ...
  const deleteDocument = async (enterpriseId: string, documentId: string) => {
    ...
    try {
      ...
      await documentService.deleteDocument(enterpriseId, documentId)
      ...
      toastStore.addToast(
        ToastType.success,
        'Documento y chunks eliminados exitosamente'
      )
    } catch (error) {
      if (deletingToastId !== undefined) {
        toastStore.removeToast(deletingToastId)
  ...
  const downloadDocument = async (enterpriseId: string, documentId: string, fileName: string) => {
    try {
      const blob = await documentService.downloadDocument(enterpriseId, documentId)
