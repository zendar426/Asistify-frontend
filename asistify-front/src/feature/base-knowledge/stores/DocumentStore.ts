...
export const useDocumentStore = defineStore('document', {
    ...
    getters: {
        ...
        totalSize: (state) => state.documents.reduce((acc, doc) => acc + parseFloat(doc.size), 0),

        documentsByType: (state) =>
            state.documents.reduce(
                (acc, doc) => {
                    acc[doc.documentTypeId] = acc[doc?.documentTypeId ?? 0] || 0 + 1
                    return acc
                },
    ...
    actions: {
        ...
        setTotalDocumentsCount(total: number) {
            this.totalDocumentsCount = total
        },
